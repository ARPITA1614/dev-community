...
RSpec.feature "UserSearches", type: :feature do
  describe "user search functionality" do
    ...
    before :each do     # used to setup necessary condition state for ur testallows to define code executed before each example in specific context   common setup done using this 
      # why we use here becz in our application we need to login user for every action
      sign_in(user)

      @user1 = create(:user, country: "India", city: "Indore")
      @user2 = create(:user, country: "Australia", city: "Sidney")
    ...
    describe "search by city" do
      ...
      it "should allow users to search other users by partial city name" do
        fill_in "q_city_or_country_cont", with: "Sid"
        click_button "Search"
    ...
    describe "view user profile" do
      it "should search user and visit profile" do
        fill_in "q_city_or_country_cont", with: "India"
        click_button "Search"
